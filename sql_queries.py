...
def select_expert_martial_artists_using_id():
    ...
    FROM hero_powers
    JOIN heroes
    ON hero_powers.hero_id = heroes.id
    WHERE power_id = 10
    ;"""
...
def select_total_damage_points_for_wonder_woman():
    return """
    SELECT SUM(powers.damage_points)
    FROM heroes
    JOIN hero_powers
    ON heroes.id = hero_powers.hero_id
    JOIN powers
...
def list_iron_mans_powers_and_respective_damage_points():
    return """
    SELECT powers.type, powers.damage_points
    FROM heroes
    JOIN hero_powers
    ON heroes.id = hero_powers.hero_id
    JOIN powers
...
def list_heroes_and_their_num_of_powers_ordered_by_hero_name_alphabetically():
    return """
    SELECT heroes.name , COUNT(hero_powers.power_id) AS num_of_powers
    FROM heroes
    JOIN hero_powers
    ON heroes.id = hero_powers.hero_id
    JOIN powers
...
def select_heroes_name_and_sum_damage_points_ordered_by_most_damage_to_least():
    return """
    SELECT heroes.name , SUM(powers.damage_points) AS total_damage
    FROM heroes
    JOIN hero_powers
    ON heroes.id = hero_powers.hero_id
    JOIN powers
